...
async fn publish_discovery(client: &AsyncClient, cfg: &MqttConfig) {
    ...
    let sensors: &[(&str, &str, &str, &str, &str)] = &[
        ...
        ("beaglebone_v2h_dc_output_volts",        "PRE DC Output Volts",
         "{{ value_json.pre.dc_output_volts | round(1) }}", "V", "voltage"),
        ("beaglebone_v2h_dc_output_amps",         "PRE DC Output Amps",
         "{{ value_json.pre.dc_output_amps | round(1) }}", "A", "current"),
        ("beaglebone_v2h_ev_max_battery_volts",   "EV Max Battery Volts",
         "{{ value_json.chademo.x100.maximum_battery_voltage | round(0) | int }}", "V", "voltage"),
        ("beaglebone_v2h_ev_target_volts",        "EV Target Volts",
         "{{ value_json.chademo.x102.target_battery_voltage | round(0) | int }}", "V", "voltage"),
        ("beaglebone_v2h_evse_output_volts",      "EVSE Output Volts (x109)",
         "{{ value_json.chademo.x109.output_voltage | round(0) | int }}", "V", "voltage"),
        ("beaglebone_v2h_pre_volts_setpoint",     "PRE Volts Setpoint",
         "{{ value_json.pre.dc_output_volts_setpoint | round(1) }}", "V", "voltage"),
        ("beaglebone_v2h_pre_dc_bus_volts",       "PRE DC Bus Volts",
...
{% else %}Normal{% endif %}"),
    ...
    let settings_numbers: &[(&str, &str, &str, &str, i64, i64, i64, &str)] = &[
        ...
        ("beaglebone_v2h_smart_export_limit_w", "v2h_smart_export_limit_w", "Smart Export Limit",   "smart_export_limit_w",     0,  6000, 100, "W"),
